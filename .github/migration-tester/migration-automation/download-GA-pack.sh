...
if echo "$response" | grep -q '"error":'; then
  ...
  echo -e "Failure in downloading Identity Server "$currentVersion" $error_description"
else
  # If there is no error, print the success message
  echo -e "Success: IS downloaded successfully."
fi

# Unzip IS archive
unzip -qq *.zip &
wait $!
echo "Unzipped  downloaded IS zip"
